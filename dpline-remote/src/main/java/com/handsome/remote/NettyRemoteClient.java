...
import com.handsome.remote.future.NamedThreadFactory;
import com.handsome.remote.future.ReleaseSemaphore;
import com.handsome.remote.future.ResponseFuture;
import com.handsome.remote.handle.NettyClientHandler;
...
public class NettyRemoteClient {
     ...
     * @param invokeCallback callback function，异步回调函数，即请求结果返回之后回调
     */
    public void sendAsync(final Host host, final Command command,
        ...
        if (acquired) {
            // 每次异步发送请求的时候，需要携带着新号量进来，总共并行能发送并同时监听200个新号量。
