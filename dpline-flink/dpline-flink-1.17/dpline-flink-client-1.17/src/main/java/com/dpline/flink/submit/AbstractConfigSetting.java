...
public abstract class AbstractConfigSetting {
    ...
    private Configuration loadConfiguration() {
        this.configuration = GlobalConfiguration.loadConfiguration(FileUtils.concatPath(submitRequest.getFlinkHomeOptions().getFlinkPath(),"conf"));
        if (!configuration.contains(DeploymentOptionsInternal.CONF_DIR)) {
            configuration.set(DeploymentOptionsInternal.CONF_DIR, FileUtils.concatPath(submitRequest.getFlinkHomeOptions().getFlinkPath(),"conf"));
        }
        return this.configuration;
    }

    /**
     * replace ${jobName} and merge flink yaml with configuration
     * @return
     */
    ...
    private void setResourceOption() {
        ...
        Integer parallelism = resourceOptions.getParallelism();
        if (Asserts.isNotNull(parallelism) && !Asserts.isZero(parallelism)) {
