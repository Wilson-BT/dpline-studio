...
import com.handsome.dao.mapper.ProjectMapper;
import com.handsome.dao.mapper.ProjectUserMapper;
...
import static com.handsome.console.util.CheckUtils.checkDesc;
...
public class ProjectServiceImpl extends BaseServiceImpl implements ProjectService {
     ...
     */
    @Override
    public Result<Object> queryProjectListPaging(User loginUser, Integer pageSize, Integer pageNo, String searchVal) {
        ...
        if (userId != 0) {
            for (Project project : projectList) {
                // 给于所有权限
                project.setPerm(Constants.AUTHORIZE_ALL_PERM);
            }
        }
    ...
    public Map<String, Object> deleteProject(User loginUser, Long projectCode) {
        ...
        FlinkTaskDefinition flinkTaskDefinition = flinkTaskDefinitionMapper.queryAllByProjectCode(projectCode);
        if(flinkTaskDefinition != null){
            putMsg(result,Status.TASK_DEFINE_EXIST_ERROR,flinkTaskDefinition.getTaskName());
            return result;
        }
    ...
    private Map<String, Object> getCheckResult(User loginUser, Project project) {
        long projectCode = project == null ? 0L : project.getCode();
        Map<String, Object> checkResult = checkProjectAndAuth(loginUser, project, projectCode);
    ...
    /**
     * query permission id，管理员有read权限，所有者有读权限
     *
     * @param user user
     * @param project project
     * @return permission
    ...
    private int queryPermission(User user, Project project) {
        if (user.getIsAdmin() == UserType.ADMIN_USER.getCode() || project.getUserId() == user.getId()) {
            return Constants.AUTHORIZE_ALL_PERM;
        }
