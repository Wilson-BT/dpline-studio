...
//    }
    ...
    public Result<Object> addJar(MultipartFile file,
        ...
        try {
            ...
            localFormatPath = genericLocalJarPath(authTypeOption.get(), projectId, mainResourceId, jarName,String.valueOf(jarId));
            // 先保存在本地
            FileUtils.copyInputStreamToFile(file.getInputStream(), jarName, localFormatPath);
            // 然后上传到S3
            ...
            FileUtils.deleteFile(localFormatPath);
            putMsg(result,Status.JAR_UPLOAD_FAILED);
            return result;
        }
        return result.ok();
...
//    }
    ...
    public String uploadJar(MultipartFile file,
        ...
        try {
            minio.putObject(uploadPath, file.getInputStream());
