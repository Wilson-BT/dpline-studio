...
import com.dpline.flink.log.model.FlinkLogEvent;
...
public class KafkaLog4j2Appender extends AbstractAppender {
    ...
    private static final Logger logger = LoggerFactory.getLogger(KafkaLog4j2Appender.class);
    ...
    protected KafkaLog4j2Appender(String name, Filter filter, Layout<? extends Serializable> layout, boolean ignoreExceptions, Property[] properties, String source, String bootstrapServers, String topic, String level) {
        ...
        if (clusterId != null) {
            ...
            if (values.length >= 3) {
                ...
                logger.error("log.file Property ({}) doesn't contains yarn application id or container id", logFile);
            }
        }
        ...
        if (clientIdPrefix != null) {
            props.setProperty("client.id", clientIdPrefix+ "_" + UUID.randomUUID());
        }
    ...
    public void append(org.apache.logging.log4j.core.LogEvent event) {
        try {
            if (level.contains(event.getLevel().toString().toUpperCase())) { //控制哪些类的日志不收集
                producer.send(new ProducerRecord<>(topic, String.valueOf(System.currentTimeMillis()), subAppend(event)));
            }
        } catch (Exception e) {
            logger.warn("Parsing the log event or send log event to kafka has exception", e);
        }
    }

    private String subAppend(org.apache.logging.log4j.core.LogEvent event) throws JsonProcessingException {
        FlinkLogEvent logEvent = new FlinkLogEvent();
        ...
        try {
            ...
            logger.error("Error getting the ip and host name of the node where the job({}) is running", appId, e);
        } finally {
            try {
