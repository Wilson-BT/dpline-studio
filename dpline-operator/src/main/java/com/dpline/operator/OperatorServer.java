package com.dpline.operator;

import com.dpline.common.minio.Minio;
import com.dpline.operator.process.*;
import com.dpline.operator.watcher.TaskStatusManager;
...
public class OperatorServer implements Closeable {
    ...
    public void runWatcher(ApplicationReadyEvent event) {
        logger.info("Starting com.dpline.operator server");
        try {
            startServer();
            // 开启循环查询任务
    ...
    private void startServer() {
        ...
        // 集群更新
        server.registerProcessor(CommandType.K8S_CLIENT_UPDATE_REQUEST,k8sClientUpdateProcessor);
