...
<template>
  <a-modal class="job-version"
    ...
    <div class="tab">
      ...
      <div :class="{'tab-checked':isTableActive === 'configCompare'}"
           @click="tabChangeEvent('configCompare')">Flink配置对比</div>
    ...
    <div class="content"
      ...
      <div class="title">
        ...
        <div class="right justify-between">
          <div>对比版本<span>(版本{{ fileTagName }})</span></div>
        </div>
      </div>
      <!-- 编辑器 -->
      <div class="container">
  ...
  export default {
    ...
    methods: {
      ...
      open (data) {
        ...
        this.fileTagName = data.fileTagName
        this.visible = true
        this.getCompareInfo()
      ...
      tabChangeEvent (type) {
        ...
        let languague = 'sql'
        // console.log(this.dataInfo[0].fileContent)
        // console.log(this.dataInfo[1].fileContent)
        let oldValue = this.dataInfo[0].fileContent
        let newValue = this.dataInfo[1].fileContent
        if (type === 'fileCompare') {
          if (this.fileType === 'DS') {
            languague = 'json'
            oldValue = this.dataInfo[0].dataStreamContent
            newValue = this.dataInfo[1].dataStreamContent
          }
        } else if (type === 'configCompare') {
          languague = 'json'
        ...
        this.$refs['moncaoEditor'].updateEditorContent(languague, oldValue, newValue)
      },
      // 获取对比信息
      async getCompareInfo () {
        let params = {
          fileTagId: this.jobId,
          fileTagName: this.fileTagName,
          compareSource: this.type,
          fileId: this.fileId
        }
        this.isLoading = true
        let res = await this.$http.post('/file/tag/compare', params)
        this.isLoading = false
        if (res.code === 200) {
          this.dataInfo = res.data
          this.fileTagName = res.data[1].fileTagName
