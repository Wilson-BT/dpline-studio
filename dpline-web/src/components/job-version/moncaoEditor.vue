...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateEditorContent(language,original,modified) {
        this.defaultOpts.language = language
        // 更新编辑器的内容
        let originalModel = this.monacoDiffInstance.getModel().original
        let modifiedModel = this.monacoDiffInstance.getModel().modified
        originalModel.setValue(original)
        modifiedModel.setValue(modified)
        monaco.editor.setModelLanguage(originalModel, this.defaultOpts.language)
        monaco.editor.setModelLanguage(modifiedModel, this.defaultOpts.language)
        // this.monacoDiffInstance.setModel({
        //           // oldValue为以前的值
