    ...
    <div class="search-table">
      ...
      <div class="data-search">
        ...
        <a-button type="primary"
                  size="small"
                  icon="plus"
                  :disabled="projectInfo.currentUserRole.isAdmin !== 1 && (!projectInfo.currentUserRole.isLeader || (projectInfo.currentUserRole.isLeader && projectInfo.currentUserRole.isLeader !== 1))"
      ...
      <chitu-table :columns="columns"
        ...
        <template #isLeader="{record}">
          <a-select style="width:120px"
                    ...
                    :disabled="(projectInfo.currentUserRole.isAdmin !== 1 && (!projectInfo.currentUserRole.isLeader || (projectInfo.currentUserRole.isLeader && projectInfo.currentUserRole.isLeader !== 1))) || record.userCode === projectInfo.currentUserRole.userCode"
    ...
    <a-modal class="add-member-dialog"
      ...
      <div class="search">
        ...
        <a-select class="auto-complete"
                  ...
                  @search="onAddSearch">
          <a-select-option v-for="(item, index) in dataSource"
                           :value="item.userCode + '-' + item.userName + '-' + item.phone"
                           :key="'user-' + index"
                           :label="item.userName"
    ...
    methods: {
      ...
      async addConfirmEvent () {
        if (this.values.length > 0) {
          ...
          let res = await this.$http.post('/project/changeRoles', params, {
            headers: {
              projectId: this.projectInfo.id
            }
          })
      ...
      onSearch (searchText) {
        if (searchText === '') {
          ...
          let numberArr = this.httpData.filter(item => item.userCode && item.userCode.search(searchText) != -1)
          let arr = nameArr.concat(numberArr)
          // 拼接后去重
          let map = new Map()
          arr.filter(item => {
            if (!map.has(item.userCode)) {
              map.set(item.userCode, item)
            }
          })
      ...
      async onAddSearch (searchText) {
        ...
        let res = await this.$http.post('/users/searchUser',
        ...
        if (res.code === 200) {
          ...
          this.dataSource.forEach(item => {
            item.disabled = false
            let arr = this.httpData.filter(tableItem => tableItem.userCode === item.userCode)
            if (arr.length > 0) {
              item.disabled = true
            }
          })
      ...
      async confirmEvent () {
        if (this.editData.length > 0) {
          await this.$http.post('/project/changeRoles', this.editData, {
            headers: {
              projectId: this.projectInfo.id
            }
          })
      ...
      memberRoleChange (value, item) {
        ...
        let arr = this.editData.filter(member => member.userCode === item.userCode)
        if (arr.length > 0) {
      ...
      async deleteMemeberEvent (userInfo) {
        ...
        let res = await this.$http.post('/project/deleteProjectUser', this.$qs.stringify(params), {
          headers: {
            projectId: this.projectInfo.id
          }
        })
        ...
        if (res.code === 200) {
          this.httpData = this.httpData.filter(item => item.userCode !== params.userCode)
          this.projectInfo.projectUsers = this.httpData
          this.editData = this.editData.filter(item => item.userCode !== params.userCode)
      ...
      async getUserList () {
        ...
        let res = await this.$http.post('/project/projectUserInfo', params, {
          headers: {
            projectId: this.projectInfo.id
          }
        })
