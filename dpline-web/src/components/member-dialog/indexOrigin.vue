  ...
  export default {
    ...
    methods: {
      ...
      async searchOfAddData (searchText) {
        ...
        if (res.code === 200) {
          ...
          this.dataSource.map(item => {
            item.text = item.userCode + ', ' + item.userName
          })
        }
      },
      // 搜索用户模糊
      ...
      selectedOfAdd (userInfo) {
        let userInfoArr = this.editData.filter(item => item.userCode === userInfo.userCode)
        let engineUsersArr = this.projectInfo.projectUsers.filter(item => item.userCode === userInfo.userCode)
      ...
      async confirmEvent () {
        if (this.editData.length > 0) {
          let res = await this.$http.post('/project/changeRoles', this.editData, {
            headers: {
              projectId: this.projectInfo.id
            }
          })
      ...
      async deleteMemeberEvent (userInfo) {
        ...
        if (userInfo.id) {
          ...
          this.editData = this.editData.filter(item => item.userCode !== userInfo.userCode)
        }
        this.tableData = this.editData.concat(this.httpData)
      },
      // 获取请求数据
