...
export const getInfo = async () => {
  ...
  // const userCode = common.getUserCode();
  if (sessionId) {
...
export const getTokenByTicket = async () => {
  ...
  if (ticket) {
    ...
    if (res.code === 200 && res.data) {
      sessionStorage.setItem('userCode', res.data.userInfo.userCode)
      localStorage.setItem('userCode', res.data.userInfo.userCode)
      store.dispatch('user/setUserCode', res.data.userInfo.userCode)
    }
    return res;
  }
};
