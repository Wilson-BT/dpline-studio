...
<script>
  export default {
    ...
    methods: {
      async login (params) {
        let res = await this.$http.post("/auth/login", this.$qs.stringify(params));
        if (res.code === 200 && res.data.sessionId != null) {
          localStorage.setItem("sessionId", res.data.sessionId);
          localStorage.setItem("userCode", params.userCode);
          this.$router.push({
            name: "Guide",
          });
        } else {
