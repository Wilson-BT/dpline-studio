        ...
        <div class="left-content justify-between">
          ...
          <p class="justify-start">
            <a-tooltip placement="topLeft">
              ...
              <div>
                项目成员数<span class="sum">{{projectTotal.userTotal}}</span>
              </div>
            </a-tooltip>
          </p>
        </div>
          ...
          return {
            children: vm.$createElement('div', {
              ...
              vm.$createElement('a-button', {
                attrs: { type: 'link', disabled: record.currentUserRole && (record.currentUserRole.isAdmin === 1 || record.currentUserRole.isLeader === 1 || record.currentUserRole.isLeader === 0) ? false : true },
                on: {
                  click: () => {
                    vm.gotoJob(record)
                  }
              ...
              vm.$createElement('a-button', {
                attrs: { type: 'link', disabled: record.currentUserRole && (record.currentUserRole.isAdmin === 1 || record.currentUserRole.isLeader === 1) ? false : true },
                on: {
                  click: () => {
                    vm.edit(record)
                  }
  ...
  export default {
    ...
    methods: {
      ...
      search (searchData) {
        ...
        this.projectName = searchData
        this.getList()
        // 直接拿着 this.projectName 去搜索
      },
      query () {
        this.pagination.current = 1
        this.getList()
      ...
      async getList () {
        ...
        if (res.code === 200) {
          if (res.data) {
            ...
            this.projectTotal.userTotal = res.data.userTotal
            if (res.data.rows) {
      ...
      gotoJob (item) {
        if (item.currentUserRole.isLeader !== 1 && item.currentUserRole.isLeader !== 0 && item.currentUserRole.isAdmin !== 1) {//不是系统管理员也不是项目成员不让他跳
      ...
      async confirmDelete (item) {
        ...
        let res = await this.$http.post('/project/delete', this.$qs.stringify(params), {
          headers: {
            projectId: item.id
          }
        })
