...
<!--        <p class="text">{{fileDetail.lockedUser}} 在 {{fileDetail.updateTime}} 锁定了当前作业</p>-->
<!--        <a-button type="primary"-->
<!--                  size="small"-->
<!--                  @click="unlock">-->
<!--          点击解锁-->
<!--        </a-button>-->
<!--        &lt;!&ndash; <a-button class="btn"-->
<!--                  @click="unlock">点击解锁</a-button> &ndash;&gt;-->
<!--      </div>-->
      <!-- 右侧配置 -->
      ...
      <div v-show="showConfig"
        ...
        <div v-show="showConfig === 'DS'"
          ...
          <DS-config ref="DSConfig"
                     ...
                     :config="fileDetail.dataStreamContent" />
        </div>
...
export default {
    ...
    watch: {
      contentData: {
        async handler (val) {
          ...
          if (fileDetail.dataStreamContent && typeof fileDetail.dataStreamContent === 'string') {
            fileDetail.dataStreamContent = JSON.parse(fileDetail.dataStreamContent)
          }
    ...
    methods: {
      changeReInitTag (val) {
        // 调用子组件的方法
        this.showConfig = "versionInfo"
        this.isMaxWidth = false
        this.$refs.versionInfoConfig.reInitPage(val)
      },
      ...
      async getSourceList () {
        ...
        let res = await this.$http.post('/system/dataSource/search', params, {
          headers: {
            projectId: Number(this.$route.query.projectId)
          }
        })
      ...
      getData () {
        ...
        // jobConfig.runResourceConfig = this.$refs.seniorConfig.runResourceConfig
        jobConfig.runMode = this.$refs.seniorConfig.runMode
        ...
        sourceConfig.jarDependList = sourceConfig.jarDependList.filter((item) =>{
          return item.mainResourceId !== "" && item.mainResourceId !== null;
        })
        sourceConfig.jarDependList.forEach((item)=>{
          item.name = '';
        });
        ...
        if (fileDetail.fileType === 'DS') {
          fileDetail.dataStreamContent = JSON.parse(JSON.stringify(this.$refs.DSConfig.dataStreamContent))
          fileDetail.dataStreamContent.name = '';
        }
        return fileDetail
      },
      ...
      //     this.$bus.$emit('queryFile', this.fileDetail.id)
