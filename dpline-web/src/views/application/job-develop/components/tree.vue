        ...
        <template v-slot:title="item">
          <span v-if="item.isFile"
            ...
            <template v-if="item.fileType === 'SQL'">
              <i v-if="item.onLine === 0"
                 class="chitutree-h5 chitutreeSQL"></i>
              <i v-if="item.onLine === 1"
  ...
  export default {
      ...
      defaultExpandedKeys: {
        handler (val) {
          if (val) {
            ...
            sessionStorage.setItem('defaultExpandedKeys_' + userCode + '_' + this.$route.query.projectId, JSON.stringify(val))
          }
        }
      },
      $route: {
    ...
    methods: {
      async init () {
        ...
        try {
          const userCode = sessionStorage.getItem('userCode')
          // sessionStorage.removeItem('defaultExpandedKeys_' + userCode + '_' + this.$route.query.projectId)
          let defaultExpandedKeys = sessionStorage.getItem('defaultExpandedKeys_' + userCode + '_' + this.$route.query.projectId)
      ...
      async moveFile (file) {
        ...
        let res = await this.$http.post('/file/removeFile', this.$qs.stringify(data), {
          headers: {
            projectId: Number(this.$route.query.projectId)
          }
        })
      ...
      async editFolder (e, item) {
        ...
        if (!flag) {
          this.$message.warning('用户无权限修改目录')
          return
        }
      ...
      async isAllowAddFile (flag) {
          ...
          if (res.code === 200) {
            if (!res.data && !flag) {
              this.$message.warning('用户无权限创建文件')
            }
      ...
      async isAllowEditFile (item) {
          ...
          if (res.code === 200) {
            if (!res.data) {
              this.$message.warning('用户无权限直接编辑文件')
            }
            return res.data
          } else {
            return true
      ...
      async getFile (folderId) {
        ...
        if(res.code === 200){
          const findItem = this.getTreeDataById(this.treeData, String(folderId))
      ...
      getTreeDataById (childs = [], foldId, type) {//通过id获取目录节点，默认获取目录节点，type=file时获取文件节点
        let finditem = null;
        for (let i = 0, len = childs.length; i < len; i++) {
          let item = childs[i]

          let id = String(item.id);
          if (id !== foldId && item.children && item.children.length > 0) {
            finditem = this.getTreeDataById(item.children, foldId)
          }
          if (type === 'file') {
            if (id === foldId && item.isFile) {
              finditem = item
            }
          } else {
            if (id === foldId && !item.isFile) {
