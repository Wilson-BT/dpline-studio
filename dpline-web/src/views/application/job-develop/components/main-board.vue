...
<!--                  v-show="isTestEnv"-->
<!--                  :loading="isChangeEnv"-->
<!--                  :disabled="activePane.contentData ? false:true"-->
<!--                  @click="changeEnv"><i class="chitutree-h5 chitutreeicon_zhuanshengchan"></i>转生产</a-button>-->
        ...
        <template v-if="activePane.contentData && activePane.contentData.fileType === 'SQL'">
          <a-button type="link"
                    :loading="isVerifying"
                    :disabled="activePane.contentData ? false:true"
                    @click="handleVerify()"><i class="chitutree-h5 chitutreeyanzheng"></i>验证</a-button>
        </template>
        <a-button type="link"
                  :loading="isOnLineing"
                  :disabled="activePane.contentData ? false : true"
                  @click="() => onlineFile('file','')"
        ><i class="chitutree-h5 chitutreeicon_shangxian"></i>{{'一键发布'}} </a-button>
        <a-button type="link"
                  :loading="isAddingTag"
                  :disabled="activePane.contentData ? false:true"
                  @click="addTag"
        ><i class="chitutree-h5 chitutreeicon_sjxz"></i>{{'TAG'}} </a-button>

<!--        <a-button type="link"-->
<!--                  :loading="isOnLineing"-->
<!--                  :disabled="activePane.contentData ? false:true"-->
<!--                  @click="handlePublishVerify"><i class="chitutree-h5 chitutreejuxing"></i>{{'发布'}} </a-button>-->
      </div>
    </div>
    <!-- end -->
    <!-- 中部 -->
            ...
            <span slot="tab">
              <div class="tab-content justify-start"
                ...
                <template v-if="item.contentData && item.contentData.fileType === 'SQL'">
                  <i v-if="item.contentData.onLine === 0"
                     class="chitutree-h5 chitutreeSQL"></i>
                  <i v-if="item.contentData.onLine === 1"
...
<!--         v-if="isHistoryFiles">-->
<!--      <a-alert show-icon-->
<!--               description="直接在生产环境开发存在一定风险，建议下线此任务,在测试环境创建作业进行开发后转至生产环境~"-->
<!--               type="warning"-->
<!--               closable-->
<!--               @close="false" />-->
<!--    </div>-->
...
<script>
  ...
  export default {
    ...
    components: {
      TabContent, addFile, addTag, OnlineProgress, TopicAuthApplyDialog,
      // ChangeEnv, ChangeProdDialog
    },
    mixins: [],
    data () {
      return {
    ...
    computed: {
      // isTestEnv () {
      //   return this.$store.getters.env === 'test'
      // },
      // isProdEnv () {
    ...
    mounted () {
      // 监听添加版本的事件
      this.$bus.$off('reInitTag').$on('reInitTag', this.reInitTagMethod)
      this.$bus.$off('onlineFile').$on('onlineFile', this.onlineFile)
      // setTimeout(() => {
    ...
    methods: {
      // 上线文件
      onlineFile(onlineType,tagId){
        console.log(tagId)
        this.$refs.onlineProgress.open(onlineType,tagId)
      },
      // 调用子组件的方法
      reInitTagMethod(){
        // console.log("接收到重置事件，并修改子组件的属性")
        this.$refs['tabContent' + this.activeKey][0].changeReInitTag(true)
      },
      // showFeatureGuide () {
      //   const guideKey = `dpline-change-env-guide`
      ...
      async addTag(){
        ...
        if (!saveResult || saveResult.code !== 200) {
          ...
          if (saveResult.flag) { // 避免kafka权限问题弹窗
            ...
            this.$refs.addTag.open({ fileId: this.activeKey})
          }
          return
        }
      },
      ...
      async init () {
        ...
        if (localFiles && String(localFiles.projectId) === String(this.$route.query.projectId)) {
          ...
          if (files) {
            ...
            if (states.code === 200 && states.data) {
              ...
              this.panes.forEach(item => {//同步文件状态
                for (let i = 0; i < enabledItems.length; i++) {
                  if (String(item.id) === String(enabledItems[i].id)) {
                    item.contentData.onLine = enabledItems[i].onLine
                    break
                  }
                }
              })
      ...
      async isAllowAddFile () {
          ...
          if (res.code === 200) {
            if (!res.data) {
              this.$message.warning('为了规范作业流程，生产环境不允许直接创建作业，建议先在测试环境进行作业开发后同步至生产环境~')
            }
            return res.data
          } else {
            return false
      ...
      async isAllowEditFile () {
          ...
          if (res.code === 200) {
            if (!res.data) {
              this.$message.warning('为了规范作业流程，生产环境不允许直接编辑作业，建议先在测试环境进行作业开发后同步至生产环境~')
            }
            return res.data
          } else {
            return true
      ...
      initActiveFile (data) {//tree双击文件时触发
        ...
        if (findItem.length === 0) {
          if (!data.etlContent && data.fileType === 'SQL') {
        ...
        //   this.autoSave()
        // }
        if (this.activePane.contentData) {
          this.autoSave()
        }
      ...
      async clickSave () {//点击保存
        ...
        if (this.$refs['tabContent' + this.activeKey][0].showConfig === 'metaTable') {
          ...
          if (res.code !== 200 && res.code !== 8515) {//submitForm code===0和code=== 8515时会触发保存
            this.handleSave()
          }
        } else {
          this.handleSave()
      ...
      async handleSave (data) {
        ...
        if (params.dataStreamConfig) {
          if (!params.dataStreamConfig.jarName) {
            this.$message.warning('DS配置中未选择JAR资源文件')
            return
          }
          if (!params.dataStreamConfig.jarVersion) {
            this.$message.warning('DS配置中未选择JAR资源版本')
            return
          }
      ...
      async handleAutoSave () {
        if (this.$refs['tabContent' + this.activeKey] && this.$refs['tabContent' + this.activeKey][0]) {
          ...
          if (!saveResult || saveResult.code !== 200) {
            if(saveResult.code === 1106){
              this.$message.error('生产环境已经锁定，禁止修改')
            }
      ...
      async handleVerify () {//点击验证
        ...
        if (!saveResult) {
          ...
          if (!verifyResult) {
            ...
            // if (isChangeEnv) this.isChangeEnv = false
            return
          } else if (verifyResult.code === 200 || verifyResult.code === 11006) {
            this.isVerifying = false
            // if (isChangeEnv) {
      ...
      //       this.isOnLineing = false
      //       return
      //     } else if (verifyResult.code === 200) {//验证成功，走发布下一步操作
      //       this.isOnLineing = false
      ...
      // },
      // 触发校验 和 发布
      async triggerPublish (form) {
        // 如果 是 form 的 type 是 file ，直接取 fileId，如果是 type 是 tag，直接去 tagId
        let res
        if(form.onlineType === 'file'){
          // 打 tag and
          const params = {
            fileId: Number(this.activeKey),
            jobName: form.jobName,
            remark: form.remark
          }
          res = await this.$http.post('/file/online', this.$qs.stringify(params), {
            headers: {
              projectId: Number(this.$route.query.projectId)
            }
          })
        } else {
          res = await this.publish(form)
        }
        ...
        if (res.code === 200) {
          ...
          this.$router.push({
            ...
            // 携带Job名称进行搜索
            this.$bus.$emit('jobOperateUpdate', form.jobName)
          })
        } else if (res.msg) {
      ...
      async save (flag) {//调用保存接口
        ...
        const data = {
          ...
          dataStreamConfig: params.dataStreamContent
        }
        ...
        if (res.code === 200) {
          if (res.data && res.data.updateTime) {
            ...
            this.saveJobTime = res.data.updateTime
          }
          for (let i = 0; i < this.panes.length; i++) {
            if (String(this.panes[i].id) === String(this.activeKey)) {
      ...
      //     remark: remark
      //   }
      //   let res = await this.$http.post('/file/tag/validateOnline', params, {
      //     headers: {
      //       projectId: Number(this.$route.query.projectId)
      //     }
      //   })
      ...
      async publish (form) {//调用发布接口
        ...
        const params = {
          // id: fileDetail.id,
          fileTagId: form.tagId,
          jobName: form.jobName,
          remark: form.remark
        }
        let res = await this.$http.post('/file/tag/online', params, {
          headers: {
            projectId: Number(this.$route.query.projectId)
          }
        })
