...
<!--          <a-select v-model="engineEnvList"-->
        ...
        <div class="form-item-value">
          <a-select placeholder="选择环境"
            ...
            <a-select-option :key="index"
                             :value="item.value"
                             v-for="(item, index) in engineEnvList">
              {{ item.label }}
            </a-select-option>
          </a-select>
        </div>
        ...
        <div class="form-item">
          ...
          <div class="form-item-value">
            <a-input v-model="formData.dataSourceUrl"
                     :disabled="type === 'detail'"
                     allow-clear
                     :placeholder="jdbcUrlPlaceholder(paramsType)"></a-input>
          </div>
        </div>
        <div class="form-item">
          <div class="form-item-label">
...
<script>
  ...
  import {envTypeList} from "@/utils/enumType";
  ...
  export default {
    ...
    computed: {
      countLength () {
        if (this.formData.description != null && this.formData.description != '') {
          return this.formData.description.toString().length
        } else {
          return 0;
        }
      },
    ...
    watch: {
      newFormData: {
        handler (value, oldValue) {
          ...
          if (newValue.databaseName !== originValue.databaseName || newValue.dataSourceUrl !== originValue.dataSourceUrl || newValue.user !== originValue.user || newValue.password !== originValue.password) {
            this.submitFlag = -1
          }
        },
        deep: true
    ...
    methods: {
      ...
      handleFormData (formData) {
        ...
        let paramsData = {}
        paramsData.user = formData.userName;
        paramsData.password = formData.password;
        if (formData.dataSourceUrl != null && formData.dataSourceUrl != '' && formData.dataSourceUrl.indexOf(this.urlPrefix) == -1) {
          paramsData.dataSourceUrl = this.urlPrefix + formData.dataSourceUrl;
        }
        return paramsData;
      },
      // 初始化
      ...
      async addDataSource (params) {
        const connectParams = this.handleFormData(params);
        let saveParams = {};
        saveParams.dataSourceName = params.dataSourceName;
        saveParams.dataSourceType = params.dataSourceType;
        saveParams.dataSourceFlag = "project";
        saveParams.description = params.description;
        saveParams.connectionParams = JSON.stringify(connectParams);
        let res = await this.$http.post('/system/dataSource/add', saveParams, {
          headers: {
      ...
      async checkConnectHttp (params) {
        this.isShowDB = false
        params = this.removeDataOfNull(params)
        let res = await this.$http.post('/system/dataSource/checkConnect', params, {
          headers: {
            projectId: Number(this.$route.query.projectId)
          }
        })
