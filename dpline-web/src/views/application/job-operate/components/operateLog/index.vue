...
<!--        </div>-->
    ...
    <div>
      <operate-log-detail ref="operateLogDetail"/>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async handleDeleteAlert(record){
        const params = {
          id: record.id,
          operateLogPath: record.operateLogPath
        }
        let res = await this.$http.post('/log/delete', this.$qs.stringify(params),{
          headers: {
            projectId: Number(this.$route.query.projectId)
          }
        })
        if (res.code === 200) {
          await this.reGetOperateLog()
          // await this.getOperateLogList(this.params)
        } else {
          this.$message.error("操作日志删除失败")
        }
      },
      async reGetOperateLog(){
        // 最面的页数只剩下一个元素的话，需要请求上一页，而不是继续请求本页
        if(this.pagination.pageRemainNum === 1 && this.pagination.current > 1){
      ...
      openLogView(record){
        if(record.operateLogPath !== null || record.operateLogPath !== ''){
          this.$refs.operateLogDetail.handleSeeLog(record)
        }
      },
      // pageSize变化回调
      ...
      resetPagination (pageInfo) {
        ...
        this.pagination.pageRemainNum = pageInfo.rowTotal === 0 ? 0 : pageInfo.rowTotal % this.params.pageSize;
      }
    }
  }
</script>
