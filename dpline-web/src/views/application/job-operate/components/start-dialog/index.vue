...
<template>
  <div v-if="isPop"
    ...
    <confirm-dialog2 :visible="isPop"
          ...
          <!-- 检查点列表 -->
          <div v-show="selectValue === 'CHECKPOINT'"
            ...
            <div class="body">
              <div v-for="(item,index) in checkList"
                ...
                <p>
                  <a-tooltip placement="topLeft">
                    <template slot="title">
                      <span>{{item.savepointPath}}</span>
                    </template>
                    <span>{{item.savepointPath}}</span>
                  </a-tooltip>
                </p>
                <p>{{item.createTime}}</p>
                <p class="delete" @click="() => deleteCheckPoint(item,'checkpoint')">删除</p>
              </div>
            </div>
          </div>
          <!-- 保存点列表 -->
          <div v-show="selectValue === 'SAVEPOINT'"
               class="save-wrapper">
            ...
            <div class="save-list"
              ...
              <div class="body">
                <div v-for="(item,index) in saveList"
                  ...
                  <p>{{item.createTime}}</p>
                  <!-- 更新 -->
                  <p class="delete" @click="() => deleteCheckPoint(item,'savepoint')">删除</p>
                </div>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    watch: {
      selectValue: {
        async handler (val) {
          this.savepointPath = ''
          if (val === 'CHECKPOINT' || val === 'SAVEPOINT') {
    ...
    methods: {
      ...
      //   }
      //   let res = await this.$http.post('/project/getProjects', params)
      //   if (res.code === 200) {
      ...
      clickCheck (record) {
        // 其他的置为 false
        this.truncateSelectPath()
        // 选择的置为true
        this.selectCheckPoint = record
        this.selectCheckPoint.checked = true
        this.savepointPath = this.selectCheckPoint.savepointPath
      },
      clickSave (record) {
        this.truncateSelectPath()
        this.selectSavePoint = record
        this.selectSavePoint.checked = true
        this.savepointPath = this.selectSavePoint.savepointPath
      },
      start () {
        if (this.selectValue === 'CHECKPOINT') {
          if (!this.savepointPath) {
            this.$message.warning({ content: '未选择检查点', duration: 2 })
            return
          }
        } else if (this.selectValue === 'SAVEPOINT') {
          if (!this.savepointPath) {
      ...
      async deleteCheckPoint(value,type){
        // 删除 checkpoint / checkpointPath
        console.log(value.savepointPath)
        let res
        if(type === 'checkpoint'){
          res = await this.$http.post("/savepoint/deleteCheckPoint", this.$qs.stringify({checkpointPath: value.savepointPath}))
        } else {
          res = await this.$http.post("/savepoint/deleteSavePoint", this.$qs.stringify({id: value.id}))
        }
        if (res.code !== 200){
      ...
      async runJobApi () {
        ...
        // 如果从消费位置开始直接启动
        // if (this.isEditConsumedPosition && this.selectValue === 'NONE') {
      ...
      async getCheckAndSavePoints () {
        ...
        const params = {
          projectId: Number(this.$route.query.projectId),
          checkpointType: this.selectValue,
          jobId: this.jobId
        }
        let res = await this.$http.post(`/savepoint/queryCheckpointAndSavepoint`, params,{
          headers: {
            projectId: Number(this.$route.query.projectId),
          }
        })
        this.isLoading = false
        if (res.code === 200) {
          const saveOrCheckList = res.data
          saveOrCheckList.map(item => {
            item.checked = false
            return item
          })
          if (this.selectValue === "CHECKPOINT"){
            this.checkList = saveOrCheckList
          } else {
            this.saveList = saveOrCheckList
          }
